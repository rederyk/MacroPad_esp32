...
void initConnectivity() {
    ...
    if (systemConfig.enable_BLE)
    {
        ...
        specialAction.setSystemLedColor(0, 0, 255, true); // Blu with saved brightness (BLE notification)
        Logger::getInstance().log("LED acceso: " + Led::getInstance().getColorLog(), true);
    }
    else
    {
        ...
        if (systemConfig.router_autostart)
        {
...

            {
                ...
                if (!systemConfig.ap_autostart)
                {
                    ...
                    specialAction.setSystemLedColor(255, 0, 0, true); // Rosso with saved brightness (WiFi failed notification)
                    Logger::getInstance().log("LED acceso: " + Led::getInstance().getColorLog(), true);
            ...
            {
                specialAction.setSystemLedColor(0, 255, 0, true); // Verde with saved brightness (WiFi connected notification)
                Logger::getInstance().log("LED acceso: " + Led::getInstance().getColorLog(), true);
        ...
        if (systemConfig.ap_autostart)
        {
            specialAction.setSystemLedColor(255, 0, 0, true); // Rosso with saved brightness (AP mode notification)
            Logger::getInstance().log("LED acceso: " + Led::getInstance().getColorLog(), true);
