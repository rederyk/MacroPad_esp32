...
// Pattern simile alle gesture: press per iniziare scansione, release per salvare

void SpecialAction::scanIR(bool pressed, int deviceId)
{
    ...
    if (pressed)
    {
        ...
        Logger::getInstance().log("IR Scan DEV" + String(deviceId) + " - Point remote and press button");
        Logger::getInstance().processBuffer();
    }
    else
    {
        // Release: check if we captured something
        if (!scanningMode)
        {
            Logger::getInstance().log("Scan mode not active");
            return;
        }

        scanningMode = false;

        // Try to get the IR signal
        if (irSensor->checkAndDecodeSignal())
        {
            ...
            {
                Logger::getInstance().log("IR captured! Press key 1-9 to save");
                Logger::getInstance().processBuffer();

                int commandKey = getKeypadInput(10000);
                if (commandKey < 0 || commandKey > 8)
                {
                    Logger::getInstance().log("Timeout - IR not saved");
                    return;
                }

                String commandName = "cmd" + String(commandKey + 1);
        ...
        {
            Logger::getInstance().log("No IR signal captured");
        }
...
void SpecialAction::sendIR(bool pressed, int deviceId)
{
    ...
    if (pressed)
    {
        ...
        sendingMode = true;
        deviceName = "dev" + String(deviceId);
        Logger::getInstance().log("IR Send DEV" + String(deviceId) + " - Press key 1-9");
        Logger::getInstance().processBuffer();
    }
    else
    {
        // Release: wait for keypad input
        if (!sendingMode)
        {
            Logger::getInstance().log("Send mode not active");
            return;
        }
        ...
        if (commandKey < 0 || commandKey > 8)
        {
            Logger::getInstance().log("Timeout - send cancelled");
            return;
        }
