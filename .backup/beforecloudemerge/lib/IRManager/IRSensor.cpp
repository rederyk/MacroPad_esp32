...
bool IRSensor::begin()
{
    if (_pin < 0)
    {
        // Logger::getInstance().log("IRSensor Error: Cannot begin(), invalid pin.");
        return false;
    }
    if (_irrecv == nullptr)
    {
        // Logger::getInstance().log("Initializing IR receiver on pin: " + String(_pin));
        _irrecv = new IRrecv(_pin);
        if (_irrecv)
        {
            _irrecv->enableIRIn();
            _lastDecodeTime = 0;
            // Logger::getInstance().log("IR Receiver enabled.");
            return true;
        }
        else
        {
            // Logger::getInstance().log("IRSensor Error: Failed to allocate memory for IRrecv.");
            return false;
        }
    }
    else
    {
        // Logger::getInstance().log("IRSensor Warning: begin() called multiple times.");
        _irrecv->enableIRIn();
        _lastDecodeTime = 0;
        return true;
    }
