...
void MacroManager::pressAction(const std::string &action)
{
    ...
    {
        specialAction.learnIRCommand(5);
    }
    else if (action == "IR_SAVE_RAW")
    {
        specialAction.saveLastIRRaw();
    }
    else if (action == "IR_SEND")
    {
        specialAction.sendIRCommand();
    }
    else if (action == "IR_CHECK")
    {
        specialAction.checkIRSignal();
    }
    else if (action == "IR_GET_LAST")
    {
        specialAction.getLastIRCommand();
    }
