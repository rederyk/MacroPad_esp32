...
public:
    ...
    void learnIRCommand(int repeats = 5);        // Learn IR command with repetitions
    void saveLastIRRaw();                        // Save last received IR signal as RAW
    void sendIRCommand();                        // Send stored IR command (interactive)
    void getLastIRCommand();                     // Get last decoded IR command
    void checkIRSignal();                        // Check current IR signal
